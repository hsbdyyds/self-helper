...
const selfAccount = ["4166454-27251362", "xxhy1990", "jd_42e72a4a4801d", "uihcc", "forgetqueque", "汤家小妹", "jd_7786d354c65f9"];
...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      if (selfAccount.indexOf($.UserName) === -1) continue;
      $.index = i + 1;
      $.isLogin = true;
      $.nickName = '';
...
async function JD818() {
  try {
    ...
    const supportNums = await supportList();//助力情况
    if (supportNums >= 100) return;
    await getHelp();//获取邀请码
      ...
      try {
        if (err) {
          ...
          if (data.code === 200) {
            ...
            resolve(parseInt(data['data']['supportedNums']));
          } else {
            resolve(0);
          }
        }
