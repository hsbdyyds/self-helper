...
const tuanUsers = ["4166454-27251362","xxhy1990","jd_42e72a4a4801d","uihcc","forgetqueque","汤家小妹","jd_7786d354c65f9"];
$.tuanIds = [];
$.tuanArr = {};
$.appId = 10001;
if ($.isNode()) {
...
async function jdDreamFactory() {
  try {
    await userInfo();
    if (!$.unActive) return
    if (tuanUsers.indexOf($.UserName) !== -1) {
      lastTuanIdx = $.index;
      await tuanActivity();
      await QueryAllTuan();
    }
    ...
    console.log(`\n账号${$.UserName} 内部相互进团\n`);
    if ($.delTuanId) {
      $.tuanIds.splice($.tuanIds.indexOf($.delTuanId), 1);
      $.delTuanId = '';
    }
    if ($.newTuanId) {
      $.tuanIds.unshift($.newTuanId);
      $.newTuanId = '';
    }
    console.log($.tuanIds);
    if ($.tuanIds.length === 0 && $.index > lastTuanIdx) {
      console.log(`无团参加，结束吧`);
      $.noTuan = true;
      return;
    }
    for (let item of $.tuanIds) {
      ...
      console.log(`\n${$.UserName} 去参加团 ${item}`);
      await JoinTuan(item);
      if ($.delTuanId && $.newTuanId) await JoinTuan($.newTuanId); // 团满开团再次加入新团
      await $.wait(1000);
    }
    // await showMsg();
  } catch (e) {
...
async function tuanActivity(ck) {
  const tuanConfig = await QueryActiveConfig(ck);
  if (tuanConfig && tuanConfig.ret === 0) {
    ...
    if (!ck) $.surplusOpenTuanNum = surplusOpenTuanNum;
    if (!tuanId && surplusOpenTuanNum > 0) {
      ...
      //查询词团信息
      const QueryTuanRes = await QueryTuan(activeId, tuanId, ck);
      if (QueryTuanRes && QueryTuanRes.ret === 0) {
        ...
        for (let item of tuanInfo) {
          ...
          if (!ck) $.tuanNum = tuanNum || 0;
          $.log(`\n开团情况:${realTuanNum}/${tuanNum}\n`);
          if (realTuanNum === tuanNum) {
            let _tuanObj = ck ? $.tuanArr[tuanId] : {};
            for (let user of userInfo) {
              let _encryptPin = ck ? _tuanObj.encryptPin : $.encryptPin;
              let _surplusOpenTuanNum = ck ? surplusOpenTuanNum : $.surplusOpenTuanNum;
              if (user.encryptPin === _encryptPin) {
                if (user.receiveElectric && user.receiveElectric > 0) {
                  console.log(`您在${new Date(user.joinTime * 1000).toLocaleString()}开团奖励已经领取成功\n`)
                  if (_surplusOpenTuanNum > 0) await CreateTuan(ck);
                } else {
                  $.log(`开始领取开团奖励`);
                  // 团长领奖
                  await tuanAward(item.tuanActiveId, item.tuanId, true, {ck, surplusOpenTuanNum: _surplusOpenTuanNum});
                  // 成员领奖
                  // if (ck) {
                  //   for (let member of _tuanObj.member) {
                  //     await tuanAward(item.tuanActiveId, item.tuanId, false, member.ck);
                  //   }
                  // }
                  // 删除已完成的团
                  delete $.tuanArr[item.tuanId];
                  $.delTuanId = item.tuanId;
                  // $.tuanIds.splice($.tuanIds.indexOf(item.tuanId), 1);
                }
              }
            }
          } else {
            console.log(item);
            const { userInfo } = item;
            let _encryptPin = $.encryptPin;
            userInfo.forEach(usr => {
              if (usr.increaseElectric === 40000 && ck) _encryptPin = usr.encryptPin;
            });
            ck ? $.newTuanId = tuanId : $.tuanIds.push(tuanId);
            $.tuanArr[tuanId] = {
              surplusOpenTuanNum,
              ck: ck || cookie,
              encryptPin: _encryptPin,
              member: []
            };
...
function QueryActiveConfig(ck) {
  return new Promise((resolve) => {
    const body = `activeId=${escape(tuanActiveId)}&tuanId=`;
    const options = taskTuanUrl(`QueryActiveConfig`, body, `_time,activeId,tuanId`, ck);
...
function QueryTuan(activeId, tuanId, ck) {
  return new Promise((resolve) => {
    const body = `activeId=${escape(activeId)}&tuanId=${escape(tuanId)}`;
    const options = taskTuanUrl(`QueryTuan`, body, `_time,activeId,tuanId`, ck);
    $.get(options, async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
...
//开团API
function CreateTuan(ck) {
  return new Promise((resolve) => {
    const body =`activeId=${escape(tuanActiveId)}&isOpenApp=1`
    const options = taskTuanUrl(`CreateTuan`, body, '_time,activeId,isOpenApp', ck);
    $.get(options, async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          ...
          if (safeGet(data)) {
            data = JSON.parse(data);
            console.log(data);
            let _tuanId = data.data['tuanId'];
            if (data['ret'] === 0) {
              console.log(`【开团成功】tuanId为 ${_tuanId}`);
              await tuanActivity(ck);
            } else {
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data['ret'] === 0) {
              ...
              // 添加团队成员信息
              $.tuanArr[tuanId]['member'].push({
                ck: cookie
              });
              console.log($.tuanArr[tuanId]);
              ...
              if (data['ret'] === 10209) {
                // 已成团，这个时候让团长领奖励
                const _tuanObj = $.tuanArr[tuanId];
                console.log(`团满了，开始领奖---${_tuanObj.ck}`);
                await tuanActivity(_tuanObj.ck);
              } else {
                console.log(`参团失败：${JSON.stringify(data)}\n`);
              }
            }
...
//开团人的领取奖励API
function tuanAward(activeId, tuanId, isTuanLeader = true, {ck, surplusOpenTuanNum} = {}) {
  return new Promise((resolve) => {
    const body = `activeId=${escape(activeId)}&tuanId=${escape(tuanId)}`;
    const options = taskTuanUrl(`Award`, body, '_time,activeId,tuanId', ck);
    let _surplusOpenTuanNum = surplusOpenTuanNum !== undefined ? surplusOpenTuanNum : $.surplusOpenTuanNum;
    $.get(options, async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          ...
          if (safeGet(data)) {
            ...
            if (data['ret'] === 0) {
              ...
              if (isTuanLeader && _surplusOpenTuanNum > 0) {
                $.log(`团奖励已领取，准备开团`);
                await CreateTuan(ck);
              }
            } else {
		...
		$.get(myRequest, (err, resp, data) => {
			try {
				...
				if (data) {
					...
					const tuanConfigs = (data[0].skinConfig[0].adConfig || [])
						.filter((vo) => !!vo && vo['channel'] === 'h5');
...
function taskTuanUrl(functionId, body = '', stk, ck) {
