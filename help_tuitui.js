...
/**
1.按照mainUsers填写的顺序依次得到助力，除非排前面的用户名助力达到要求，不然后面账号得不到助力
...
const $ = new Env('***京东极速版---推推赚大钱***'); 
const notify = $.isNode() ? require("./sendNotify") : "";
 ///**中间为需要设置的参数
const actId ="287eb90945e049129d76dd7e85dc0313";
$.self = ["4166454-27251362", "xxhy1990", "jd_42e72a4a4801d", "uihcc", "forgetqueque", "汤家小妹"];
...
$.CryptoJS = $.isNode() ? require('crypto-js') : CryptoJS;
 ...
 !(async () => {
   ...
   for (let i = 0; i < $.cookieArr.length; i++) {
      ...
      if($.self.indexOf($.userName)<0){continue;}            
      $.index = i + 1;
      $.isLogin = true; //ck检测
      await TotalBean();//ck检测
      if(!$.isLogin){continue} //ck检测              
      $.log(`\n开始【京东账号${i + 1}】${$.userName}****************************************开启红包****************************************`);
      ...
      if(mainUsersArr.length === $.self.length){break;}
   }
   for (let i = 0; i < $.cookieArr.length; i++) {
    $.currentCookie = $.cookieArr[i];
    if ($.currentCookie){
      $.userName = decodeURIComponent($.currentCookie.match(/pt_pin=(.+?);/) && $.currentCookie.match(/pt_pin=(.+?);/)[1]);
      if($.self.indexOf($.userName)>=0){continue;}//自己的账号不助力
      $.index = i + 1;
      $.isLogin = true; //ck检测
      await TotalBean();//ck检测
      if(!$.isLogin){continue} //ck检测     
    ...
    $.post(options, async (err, resp, data) => {
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            data = JSON.parse(data);
            console.log(`本次助力${name}；互助码：${$.packetID}`);
            ...
            if (data.code === 0){
              ...
              console.log(`${$.userName} :helpCoinDozer msg:${data.msg},code:${data.code}\n`)
            }
          }
        }
      } catch (e) {
