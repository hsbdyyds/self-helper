...
const inviteCodes = ['Jxoxaui3YuEk9mzQynAV1A','a1NvJe27b_w','eU9YaO7nYf539T_Rw3MSgg','ZkJvP78','dUR1O7n2J7lzsCuA','eU9Ya-u6YKgl9GqGzXZF3w','9ZqjuXI0s3yZJPhc','IhM2ZO-yYvkh8m66iw','clh3M7M']
if ($.isNode()) {
...
async function jdCash() {
  await index();
  $.newShareCodes = inviteCodes.map(code => {
    return { "inviteCode": code, "shareDate": $.shareDate }
  });
  // await $.wait(1000);
  await helpFriends();
  // await $.wait(1000);
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if( data.code === 0 && data.data.bizCode === 0){
              ...
              if (data.data.bizCode===208) {
                let idx = inviteCodes.findIndex(ivc => ivc === helpInfo.inviteCode);
